...
type TriggerInference(helper:Helper.Env, bodies:Lazy<list<ToBoogieAST.Function>>, quantTok:Token, invMapping:Dict<B.Expr,list<C.Expr>>, quantVars:list<B.Var>) =
  ...
  let rec matchExpr (sub : Map<_,_>) (a, b) = 
    ...
    if helper.Options.DumpTriggers >= 10 then
      Utils.Log(String.Format("  unify: {0} / {1} ?= {2} -> {3} )))", a, substToString (Some sub), b, substToString res))
    res
  ...
  let doInfer (body:B.Expr) =
    ...
    if dbg then
      Utils.Log(String.Format("infer: {0}", quantTok.Value))
      Utils.Log(String.Format("  BPL: {0}", body.ToString()))
    for (tr, vs, q) in candidates do
      if List.forall vs.ContainsKey quantVars then
        if dbg then Utils.Log(String.Format("  cand: {0} q:{1}", tr, q))
        if willLoop tr then ()
        elif q >= 0 then
          resTrig.Add (q, [tr])
      ...
      let rec loop mtrig =
        if coveredVars.Count = quantVarSet.Count then
          ...
          let checkCandidate cur (tr, (vs:Dict<_,_>), q) =
            ...
            if dbg then Utils.Log(String.Format("  check multi: {0} q:{1} v:{2}", tr, q, addedVars))
            if addedVars > 0 then
              match cur with
          ...
          match Seq.fold checkCandidate None candidates with
            | Some (tr, q, av, vs) ->
              if q <= strigQual then ()
                ...
                if dbg then Utils.Log(String.Format("add multi: {0} q:{1} v:{2}", tr, q, av))
