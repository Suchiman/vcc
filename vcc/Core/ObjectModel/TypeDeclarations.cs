...
namespace Microsoft.Research.Vcc {
  ...
  class VccExtensionFields : NamespaceTypeDeclaration
  {
    private VccExtensionFields(NameDeclaration name, List<ITypeDeclarationMember> members) :
      base(null, Flags.None, name, new List<GenericTypeParameterDeclaration>(0), new List<TypeExpression>(0), members, SourceDummy.SourceLocation) {
    }
    ...
    public ITypeDeclarationMember/*?*/ this[string fieldName] {
      get { 
        var members = this.GetTypeDeclarationMembersNamed(this.Compilation.NameTable.GetNameFor(fieldName).UniqueKey); 
        if (IteratorHelper.EnumerableIsEmpty(members)) return null;
        return IteratorHelper.First(members);
      }
    }

    public static VccExtensionFields CreateInstance(VccCompilation compilation) {
      VccExtensionFields result;
      NameDeclaration name = new NameDeclaration(compilation.NameTable.GetNameFor("\\ExtensionFields"), SourceDummy.SourceLocation);
      result = new VccExtensionFields(name, GenerateBuiltInFields(compilation.NameTable));
      compilation.ExtensionFields = result;
      return result;
    }

    private static List<ITypeDeclarationMember> GenerateBuiltInFields(INameTable nameTable) {
      var setTypeName = new VccNamedTypeExpression(NamespaceHelper.CreateInSystemDiagnosticsContractsCodeContractExpr(nameTable, "Objset"));
      var objTypeName = new VccNamedTypeExpression(NamespaceHelper.CreateInSystemDiagnosticsContractsCodeContractExpr(nameTable, "TypedPtr"));
      var ownsField = GenerateBuiltInField("\\owner", objTypeName, nameTable);
      var ownerField = GenerateBuiltInField("\\owns", setTypeName, nameTable);
      var result = new List<ITypeDeclarationMember>(2);
    ...
    private static FieldDefinition GenerateBuiltInField(string name, VccNamedTypeExpression type, INameTable nameTable) {
      var result = new Vcc.FieldDefinition(new List<Specifier>(), (FieldDeclaration.Flags)0, type, new NameDeclaration(nameTable.GetNameFor(name), SourceDummy.SourceLocation),
        null,
        true,
