...
\noindent
The syntax \vcc{\forall unsigned i; i < sz; ar[i] != elt} means that 
\vcc{ar[i] != elt} for all \vcc{i} between \vcc{0} and \vcc{sz - 1}.
The \vcc{0} bound comes from the fact that we're quantifying over and unsigned
integer.
