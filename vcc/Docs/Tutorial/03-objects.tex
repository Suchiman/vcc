...
of that struct will shorter than \vcc{SSTR_MAXLEN} and \vcc{'\0'}-terminated.
...
(these annotations are explained in \secref{reint}).
...
tmp = a->x;
\end{VCC}
VCC checks that the \vcc{&a->x} points to currently valid \vcc{int}.
...
Let's then have a look at how do we initialize an instance of the \vcc{SafeString} struct.
...
You could ask why wouldn't the invariant always hold? Already the \vcc{sstr_init()}
