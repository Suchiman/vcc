...
_(unwrap(o)); { ... } _(wrap(o));
\end{VCC}
Thus, what happens in the example is that we first unwrap the string, then its content, do the
updates, and then wrap them again.
The interesting thing is that only \vcc{s} is listed in the writes clause.
