...
maintains a count of them.  You would then need to include \emph{ghost
  data} to store this set and write small bits of \emph{ghost code} to
...
homepage\footnote{\url{http://vcc.codeplex.com/}}. Because this is a
tutorial, we will occaisionally provide a simplified (and technically
incorrect) explanation of what is going on, providing some additional
clarification in the footnotes, which can be skipped on first reading.
...
\begin{note}
  ...
  \footnote{
    Footnotes, on the other hand, will be usually used when something
    stated in the main text is a simplification.
  }
\end{note}
