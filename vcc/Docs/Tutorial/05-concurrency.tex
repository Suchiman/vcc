...
typedef struct {
  \ptrset claimed;
  _(invariant \forall \object o; o \in claimed ==> o->\consistent)
} \claim_struct, *\claim;
)
\end{VCC}
...
claims.
The number of outstanding claim on an object is stored in
...
Additionally, the \vcc{InitiliazeLock()} needs to ensure \vcc{l->\claim_count} \vcc{== 0}
...
inconsistent objects, so we know that \vcc{q} is consistent in both \vcc{S0}
and \vcc{S1}.
...
To prove \vcc{\active_claim(c)} one needs to prove \vcc{c->\consistent} and that
