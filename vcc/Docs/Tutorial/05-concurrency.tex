...
Creating or destroying a claim on \vcc{c} requires write access to \vcc{\claim_count(c)}.
One way to obtain such access is getting sequential write access to \vcc{c} itself.



The \vcc{true} in \vcc{\make_claim(...)} is the claimed property (an invariant of the claim), which will be explained
in the next section.
