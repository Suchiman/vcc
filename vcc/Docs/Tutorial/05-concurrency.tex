...
claims.
The number of outstanding claim on object \vcc{p} is given by
the \vcc{\claim_count(p)} function.
...
Additionally, the \vcc{InitiliazeLock()} needs to ensure \vcc{\claim_count(l)} \vcc{== 0}
