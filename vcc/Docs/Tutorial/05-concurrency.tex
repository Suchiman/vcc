...
A call to the initialization will look like \vcc{InitiliazeLock(&l _(ghost o))}.

Second, we require that the object to be protected is wrapped (recall that wrapped means consistent
...
preconditions on the \vcc{InitiliazeLock()}.
Note that the \vcc{Release()} does not mention the lock it its writes clause,
...
\itodo{we should actually do that}
\end{note}
...
Additionally, the \vcc{InitiliazeLock()} needs to ensure \vcc{\claim_count(l) == 0}
...
The \vcc{InitiliazeLock()} leaves the lock wrapped and writable by the current thread.
This allows for creation of claim, which is then passed to \vcc{Acquire()} and \vcc{Release()}.
...
This would be the case if \vcc{n} was mutable, but it is wrapped.
...
The third assumption follows from the second (if \vcc{o} did change, its invariant would
automatically hold).
...
If \vcc{o} owns \vcc{p}, then \vcc{p \in o->\owns}.
By the third admissibility assumption, after the simulated action \vcc{o} still owns \vcc{p}.
By the rules of ownership (\secref{wrap-unwrap}), only threads can own
inconsistent objects, so we know that \vcc{p} is consistent in both \vcc{S0}
and \vcc{S1}.
Therefore non-volatile fields of \vcc{p} do not change between \vcc{S0} and \vcc{S1},
and thus the invariant of \vcc{o} can freely talk about their values:
whatever property of them was true in \vcc{S0}, will also be true in \vcc{S1}.
Additionally, if \vcc{p} owned \vcc{q} before the atomic action, and the \vcc{p->\owns} is non-volatile,
