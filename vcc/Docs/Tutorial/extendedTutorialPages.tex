...
verify. The \Q{/Inspector} switch causes the \VCC\ inspector to br
...
constant), \Z\ is good at proving that there is no way to simultanously
...
semicolon would be ommitted if this declaration contained a body
also. )

\VCC\ translates these specs as assuming the preconditions at the
...
within a psotcondition, \Q{old(e)} gives the value that the expression \Q{e}
