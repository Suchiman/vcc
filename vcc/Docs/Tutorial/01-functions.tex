...
The assumption \vcc{_( assume E )} can be viewed as telling VCC to ignore
the rest of an execution if \vcc{E} fails to hold (\ie, where 
\vcc{E} evaluates to \vcc{0}). To put it another way, if you verify a
...
assumption fails, no assertion will fail either.  Of course
assumptions, like all annotations, are not included in the executable,
so if an assumption fails the program doesn't stop and the following
assertion might get violated.  Thus, you normally want to verify your
program without any assumptions.
...
be \vcc{\thread_local}. Whenever a memory object is read, VCC adds an
assertion to this effect. 

Writing requires something more; by default, VCC assumes that a
function call has no side effects on its caller; for example, we
...
In VCC, the destruction of knowledge of the caller is specified by
means of yet another form of annotation, called \emph{writes clauses}.
...
\vcc{*p} is writeable, in annotations, this is written as
\vcc{\writeable(p)}. We give details later, but
roughly speaking, this means
\begin{itemize}
\item \vcc{p} belongs to the current thread and is in a ``phase of
  life'' where it can be written; such objects are said to be
  ``unwrapped''. (Note that this also implies \vcc{\thread_local(p)}.)
\item Either \vcc{p} was listed in the writes clause of the function,
  or was unwrapped after the function was entered; this guarantees
  that the caller is going to forget what he knew about the value of
  \vcc{*p}%
\end{itemize}
...
made unwritable (using mechanisms to be described later). 
VCC asserts \vcc{\writable(p)} on each attempt to write to \vcc{*p},
as well as on each call to a function that lists \vcc{p} in a writes
clause. 

\todo{Change this to a function that just copies from one variable to
  the other, so the copied variable can be required to be \vcc{thread_local}.}
...
annotations. These annotations are particularly useful when specifying
programs that operate on arrays; we'll see many more applications of
these extensions later.

Within a VCC declaration, the type \lstinline|T[E]| where \vcc{T} is
