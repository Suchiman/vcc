...
\noindent
VCC says that \vcc{main()} is correct, only \vcc{min()} is broken.
...
Intuitively, a clause \vcc{spec( writes p, q )} is equivalent to saying
...
saying that everything except for \vcc{p} and \vcc{q} stays unchanged.
...
\vccInput[linerange={beg-foo}]{c/01_writes1.c}
...
Now, let's have a look at a function calling \vcc{boundedIncr()}.

\vccInput[linerange={foo-}]{c/01_writes1.c}

There are two things to note here.
First, one lists pointers to memory locations, not l-values in the writes
clause.
This is why we write \vcc{spec(writes &x, &y)} and not \vcc{spec(writes x, y)}.
Second, VCC has no problems showing that \vcc{boundedIncr(&x)} does not change
\vcc{y}.

Should we forget the writes clause on the \vcc{boundedIncr()}, we would get an error:

\vccInput[linerange={out-}]{c/01_writes2.c}

\noindent
The first error refers to the read access to \vcc{*p} in the condition of the
\vcc{if} statement.
It is generally unsafe to read something just because you happen to have a pointer to it.
For example, the pointer might be \vcc{NULL}, or the object pointed to might be lock-protected.
It might have also been \vcc{free()}d.
For these (and other) reasons VCC inserts an implicit assertion of the form
\vcc{\thread_local(p)} just above any read access to \vcc{*p}.
Similarly, for write access to \vcc{*p} it inserts the assertion
\vcc{\writable(p)}.
For example, the assert/assume translation of \vcc{boundedIncr()} is:

\vccInput[linerange={beg-}]{c/01_writes3.c}

\noindent
Because the clause \vcc{writes p} essentially gives rise to a precondition
\vcc{requires \writable(p)}, when one calls \vcc{boundedIncr(p)},
VCC will also insert \vcc{assert \writable(p)}.
In particular if we forget to list \vcc{&x} in the writes
clause of \vcc{foo()} we will get:

\vccInput[linerange={out-}]{c/01_writes4.c}
...
%For example, if we remove the \vcc{spec( writes ... )} clause from the
