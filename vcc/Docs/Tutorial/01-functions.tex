...
A simplest way of doing that is through \vcc{_( assert E );}
...
All such annotations, surrounded by \vcc{_( ... )}
...
\end{itemize}
These are given with \vcc{_( requires E )} and \vcc{_( ensures E )} respectively.
...
Thus we say \vcc{_( requires \true )} to say that the entry condition is
always true.
...
We've already seen assertions, that is annotations of the form \vcc{_( assert E )}.
...
An assumption, written \vcc{_( assume E )}, has only the second component:
...
An assumption of the form \vcc{_( assume \false )} makes everything below
...
\vcc{_( assume 1 != 0 )} or \vcc{_( assume x < 0 )} when \vcc{x} is \vcc{unsigned}.
...
\vcc{requires \writable(p) && \writable(q)}, when one calls \vcc{swap(p, q)},
VCC will also insert \vcc{assert \writable(p) && \writable(q)}.
