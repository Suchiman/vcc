...
\end{VCC}
\noindent
\todo{Should we make \vcc{havoc} part of the language?}
...
In the postcondition the syntax \vcc{\old(E)} returns value
of expression \vcc{E} as if it was evaluated before the function
started, in the so-called \Def{pre-state} of the call.
Thus, our postcondition states that the new value of \vcc{*to}
equals the value of \vcc{*from} before the call.
In fact, the postcondition \vcc{*to == *from} is equivalent,
because the function does not write \vcc{*from}.
