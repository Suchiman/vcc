...
threads.
You can safely ignore that until \secref{concurrency}.
}
In a sense this is similar to the macro \lstinline|assert(E)| defined in the \lstinline|<assert.h>|
header file --- the macro aborts the program when \vcc|E| evaluates to non-zero value.
The difference is that the runtime macro works just for the particular input
that you happen to supply when running your program, 
whereas VCC will \emph{prove} that the assertion holds for \emph{all} executions.

Getting back to our example,
the line \vcc{_( assert z <= x && z <= y )} says
that when control reaches the assertion, \vcc{z} is no larger than \vcc{x} or
\vcc{y}. 
To verify this function using VCC save the source in a file called \verb|minimum.c|
and then run:
