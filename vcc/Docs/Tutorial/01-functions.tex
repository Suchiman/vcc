...
\vcc{E} is guaranteed to hold\footnote{
  ...
  theads; see section \secref{concurrency}}.  
...
doesn't hold. First, |assert(E)| requires runtime overhead (at least
...
where the assumption fails to hold; thus, when VCC succesfully
...
So far, we have considered only functions without visible side edfects; for
...
that of the memory objects ``local'' to the caller before the call,
...
\footnote{
  ...
  guaranteed to remain in writable.
}:
