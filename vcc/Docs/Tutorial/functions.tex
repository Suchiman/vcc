...
The expression \vcc{\thread_local_array(ar, sz)} essentially translates to 
the the conjunction of \vcc{\thread_local(ar[i])} for each \vcc{i<sz}. 
...
\footnote{The other useful idiom is to require that the read object
  ...
  remains thread-local throughout the function; see \secref{ownership}.}
...
note one special case: the expression \vcc{\array_range(arr, len)}
denotes the set of pointers \vcc@{&arr[0], &arr[1], ..., &arr[len-1]}@. 
Thus, a writes clause of the form
\vcc{_(writes(\array_range(arr,len)))}
...
adding the modifier \vcc{_(vcc ispure)} to the function specification,
\eg
\begin{VCC}
