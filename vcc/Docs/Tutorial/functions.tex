...
For example, the function below checks if an array is sorted:
\vccInput[linerange={begin-end}]{c/01_issorted.c}
It requires that array to be thread local,
and ensures that its result is true if and only if
the array is indeed sorted,
that is for any \vcc{i < j} in the array \vcc{arr[i] <= arr[j]}.
The operator \vcc{<==>} means the same as \vcc{==} on Booleans,
but has very low priority. It's usually read as ``if and only if''.

The function is implemented by checking if the last two elements of the
array are in order, and if so the array is sorted if the array
one shorter is sorted.
