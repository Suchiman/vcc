...
the identity
of an object is given by its \Def{address} and its \Def{type}. 
(For example, in a C program including a user-defined \vcc{struct}
type, there will be an object for each address such that an object of
that type would be properly aligned.)
Each
...
objects. This implies that every legal execution of good program is
simulated by a legal execution in which concrete field accesses are replaced
...
the projection of a legal execution of the program, which allows properties
...
\vcc{\diff}  \>$\cup$\>left \> set difference\\
\vcc{\subset}\>\>left \> subset test\\
\end{tabbing}
...
The type of claims. 
\\\\
A compound type (real or ghost) can be marked a \vcc{_(claimable)} (just before
...
Example: \vcc{\forall unsigned i; i <= 10 ==> a[i] < 10}.
