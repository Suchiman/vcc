...
\\\\
\vcc{_(retype)} (cast)\\
This operator is well-defined only when operating on a pointer \vcc{p} to a
...
\\\\
For object \vcc{o} with field \vcc{f} of primitive type \vcc{T}, 
...
stuct <ObjectType> {
  ...
  _(invariant \claim_count > 0 ==> \closed)
  _(invariant \approves(\owner,\claim_count))\end{VCC}
...
\\\\
\begin{VCC}
_(pure) \bool \inv2(:pure \object o)
{ \return \inv2s(\old(\now()),\now(),o); }
\end{VCC}
\\\\
\begin{VCC}
...
\footnote{Caveat: currently, \vcc{:lemma} invariants of the form 
...
\\\\
\begin{VCC}
coid \join_arrays(\object a1, \object a2)} (statement)\\
...
type, with starting address the same as that of \vcc{a1}, and length
...
while  \vcc{o->\closed} \footnote{Caveat: \vcc{\volatile_version}
