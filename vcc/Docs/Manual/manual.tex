...
stuct {
  ...
  _(invariant :\ownerOb \unchanged(\owner) 
                        || ((\inv2(\old(\owner)) || <\old(\owner) not of compound type>)
                            && (\inv2(\owner) || <\owner not of compound type>)))
  ...
  _(invariant \closed || \old(\this->\closed) && <\this of compound type> ==> \inv2(\this))
  ...
  _(invariant <this is a user-defined compound type> ==>
     ...
     && (\forall <field f>;
         ...
         && (<\approves(e,f) is a top-level conjunct of an invariant of this type>
               ==> \unchanged(f) || <\owner not compound> || \inv2(\owner)))
\end{VCC}
...
\vcc{\bool \inv2s(\pure \state s1,\pure \state s2,\pure \object o)}\\
...
user-defined type invariants in the definition of the type of \vcc{o},
with \vcc{\old(e)} replaced by \vcc{\at(s1,e)} and
with \vcc{o} substituted for \vcc{\this}.
\\\\
...
\vcc{_(invariant \bool \pure e)} (member)\\
...
\vcc{T \this} (pure)\\
An ocurrance of a name \vcc{f}, not beginning with \vcc{\}, that is
