...
Text in \vcc{this type} is written in this language, with the
...
should precede any annotations (after expansion of \vcc{#include}
directives). 
 
VCC reserves (in both the program and the unannotated program) 
the preprocessor macro names \vcc{VERIFY}\footnote{
Use of \vcc{VERIFY}, 
The use of \vcc{#ifdef VERIFY}
}, \vcc{_VCC_H},
and the macro \vcc{_(...)}\footnote{
Caveat: VCC currently makes use of a number of other preprocessor
macro names, all of which are defined in \vcc{<vccp.h>} in the VCC
...
VCC adds the following operators, which can appear in expressions
(inside of annotations):\\\\
\vcc{ ==>} (infix, right associative, precedence 2)\\
\vcc{ <==} (infix, right associatibe, precedence 2)\\
\vcc{ <==>} (infix, right associative, precedence 2)\\
\vcc{ \in}  (infix, right associative, precedence 6)\\ 
\\
In addition, there is an additional syntaxtic productions for
expressions:
\begin{VCC}
...
of a compound type definition, between thejust before the \vcc{struct} or \vcc{union}
keyword;
...
are not permitted. Such contexts are marked \vcc{\pure} in this
manual; this keyword does not apepar in annotations. 
...
declaration;
\item[invariant] can only appear in object invariants;
...
\vcc{[\addr(p),\addr(p) + \sizeof_object(p))}.
\\\\
\vcc{e \is T} (expr)\footnote{Deprecated}\\
\vcc{e} must be a pointer, and \vcc{T} a primitive type. 
True iff \vcc{\typeof(e)} is \vcc{T}.
...
stuct {
  ...
  _(invariant \forall \object o1; \this->\closed && o1 \in \this->\owns ==> o1->\owner == \this)
  _(invariant :\ownerOb \unchanged(\owner) || (\inv2(\old(\owner)) && \inv2(\owner)))
  _(invariant :\ownerOb \owner->\closed)
  _(invariant :\ownerOb \this \in \owner->\owns)
  _(invariant :\ownerOb \claim_count > 0 ==> \closed)
  _(invariant :\ownerOb \approves(\owner,\claim_count))
  ...
  _(invariant \closed || \old(\this->\closed) ==> \inv2(\this))
  _(invariant \old(\version) <= \this->\version)
  _(invariant \old(\volatile_version)  <= \this->\volatile_version)
  _(invariant \unchanged(\version) ==> \unchanged(\this->\closed))
  _(invariant \closed || \old(\closed) ==> \inv2s(\old(\now()), \now(), \this))

  // the following for every nonvolatile field f
  _(invariant \closed && \old(\closed) ==> \unchanged(\this->f))
\end{VCC}
If \vcc{T} is a user-defined struct or union type, and \vcc{\approves(\this->\owner,f)}
is a top level conjunct of a user-defined invariant of the type, then
the following invariant is included:
\begin{VCC}
  _(invariant \unchanged(\volatile_version) || !\closed(\this) || \unchanged(f))
\end{VCC}
If \vcc{T} is a user-defined struct or union type, and \vcc{\approves(e,f)}
is a top level conjunct of a user-defined invariant of the type, then
the following invariant is included:
\begin{VCC}
  _(invariant \unchanged(f) || \owner \is \thread || \inv2(\owner))
\end{VCC}

\begin{VCC}
}
\end{VCC}
The fields \vcc{\version} and \vcc{\volatile_version}, and the
group \vcc{\ownerOb}, are not allowed in annotations. Also, in
annotations, fields beginning with \vcc{\} can appear only following
the \vcc{->} operator\footnote{This restriction should be
eliminated.}. 
\\\\
...
object, the result is undefined. Within an object invariant, this
function can appear only with positive polarity.  
\\\\
\noindent\vcc{\bool \inv2(\object o)}\\
True iff \vcc{\inv2s(\old(\now()),\now(),o)}.
\\\\
...
object \vcc{o} that is owned by a thread
\label{sec:approval}
\vcc{_(bump_volatile_version o)} (statement)\\
This translates to 
\begin{VCC}
{
  \natural n;
...
\end{VCC}
This is the only operation that changes the \vcc{\volatile_version}
field of an object. This restriction allows \vcc{o->\volatile_version}
to be treated essentially as \vcc{o->\version}. 
\\\\
...
\noindent\vcc{\bool \wrapped(\object o)} (thread)\\
True iff \vcc{o->\owner == \me && o->\closed}.
\\\\
\noindent\vcc{_(dynamic_owns)} (compound)\\
\vcc{_(volatile_owns)} (compound)\\
These cannot both appear on the same type definition. If a compound
type declaration is not marked \vcc{_(volatile_owns)}, then the type
gets as an implicit invariant
\begin{VCC}
  _(invariant \this->\closed ==> \unchanged(\this->\owns))
\end{VCC}

If, in addition, the type is not marked \vcc{_(dynamic_owns)}, then
...
\end{VCC}
In this case, \vcc{_(wrap o)} automatically sets \vcc{o->\owns} to
...
For primitive pointer \vcc{o}, this translates to \vcc{o* == \old(o*)} 
...
owner-approved.
\\\\
\vcc{void \join_arrays(\object a1, \object a2)} (statement)\\
...
\\\\
\vcc{void \split_array(\object a, \size_t s)} (statement)\\
\vcc{a} must be a \vcc{\mutable}, \vcc{\writable} array object. This
statement asserts that \vcc{a->\length < s}, invalidates the
...
The result of the cast is \vcc{\blob(o, s)}.
\\\\
\vcc{_(blob_of)} (cast)\\
The argument must be an object.
...
The type of claims. 
\\\\
A compound type (real or ghost) can be marked a \vcc{claimable} (just before
...
_(assume <s is the set of claimed objects of c 
  and \vcc{p} is the invariant of s>)
...
\\\\
\noindent\vcc{_(by_claim(\claim c)} (cast)(thread)\\
...
Ghost declarations are enclosed in \vcc{_(ghost)}.  Top-level ghost
...
object. If a global variable \vcc{v} of primitive type is declared as
...
typedef struct Thread {
  ...
  _(invariant \unchanged(\currentActivation) 
       || (\inv2(\currentActivation) && \inv2(\old(\currentActivation))))
  _(invariant \forall \object o; \unchanged(\mine(o)) || \inv2(\currentActivation->\code))
...
typedef struct Activation {
  ...
  _(invariant \unchanged(\currentWritesSet) || \inv2(code))
  ...
  _(invariant \unchanged(done) || \inv2(\code))
} Activation;
...
\vcc{_(updates \object o)} (contract)\\
Equivalent to
\begin{VCC}
...
\vcc{_(out_param(\object p)} (contract)\\
Equivalent to
\begin{VCC}
  _(writes p)
...
\vcc{_(unwrap \pure \object o, ...)} (statement)\\
...
\vcc{_(unwrapping \object o1, \object o2, ...) writes block} (statement)\\
...
Here, \vcc{stuff} is a declaration or a statement. If a declaration,
it does not have to precede all statements in the current block.
\vcc{stuff} follows the syntax of C statemnets, but can use the
operators and functions described in this document. \vcc{stuff} does
not have to be terminateed by a semicolon.

\subsection{Conditional Statements}
The conditional \vcc{if (p) then S1}, where \vcc{p} is a local
variable,
...
contracts. It is true iff \vcc{o} is in the writes set of the current
