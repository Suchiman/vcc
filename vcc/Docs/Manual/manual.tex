...
program, which we refer to as ``the program''. ``The unannotated
...
The program defines a fixed set of typed \Def{objects}, each with a
collection of fields. A state of the program is given by the values of
each of the object fields. Each object has a
two-state \Def{invariant} (a predicate on ordered pairs of states, a
prestate and a poststate), determined by its address and type; these
invariants can mention arbitrary parts of the state. In this manual,
the invariants of each type are presented by requirements of the form
``this is an invariant of this type''; the invariant of an object is
the conjunction of all invariants mandated herein.

A 2-state invariant can be interpreted as an invariant on a
single state by applying it to the transition from the state to itself
(\Def{stuttering}). A state is \Def{good} if all object invariants
hold in that state; a state transition is good if it satisfies every
object invariant. A sequence of states (finite or infinite) is good
iff all of its states and transitions are good.

A transition modifies an object if some field of the object differs
between the prestate and poststate of the transition. A transition
is \emph{legal} if its prestate is not good or if it preserves the
invariants of all modified objects. An execution of the program is a
sequence (finite or infinite) of states, such that the initial state
...
it is good. The program actually verified includes additional
``ghost'' code and data not present in the unannotated program;
however, succesful verification implies that every execution of the
unannotated program (starting from a projection of a good state) is a
projection of an execution of the program\footnote{This is because
verification proves that all ghost code terminates, and does not
change the concrete state.}. 

The invariant of object \vcc{o} is \Def{admissible} iff (1) every legal
state transition, starting from a good state, satisfies the invariant
of \vcc{o}, and (2) every good transition, starting from a good state,
results in a state from which stuttering satisfies the invariant
of \vcc{o}. A type is admissible if the invariant of each object of
that type is admissible. The program is verified by proving that every
type of the program is admissible. 

A running instance of a function is represented in the model by a
function activation object. Such an object has fields giving values of
the local variables of the function, as well as a program counter that
gives the location in the code where control of the activation
sits. One (trivially admissible) invariant of a function activation is
that if its program counter changes, then the change to the global
state is given by the operational semantics of the program step being
taken. A second invariant of a function activation, constructed
according to the annotation of the function, says what should be true
at each control point of the function; this invariant (which
corresponds to a ``noninterference proof'' for the function
annotation) is admissible by its construction. 
The third invariant of a function activation is that any
transition from the current state allowed by the operational semantics
of the function text is legal; this invariant guarantees every
sequence of program steps is a legal execution.  Proving admissibility
of this invariant corresponds to proving ``local correctness'' for the
function.

Each concrete object field has a fixed heap location and
representation size.  Each object has a field that indicates if the
object is valid, and an invariant saying that if it is valid, then its
fields don't overlap the fields of other valid objects. An invariant
of each function activation is that it reads and writes only valid
objects. This implies that every execution of good program is
simulated by an execution where concrete field accesses are replaced
by heap accesses, which gets us back to conventional C program
semantics.
...
The program text determines a fixed set of \Def{pointers}.  Each
pointer is either primitive or an \Def{object}. 
A pointer is characterized by the following (state-independent) parameters:
\\\\
\vcc{\natural \address(\object p)}\\
The address of \vcc{p}. If \vcc{!\ghost(p)}, \vcc{\address(p) < UINT_PTR_MAX}.
\\\\
...
The object of which \vcc{p} is a part of. If \vcc{p} is itself an object, the
result is undefined. If \vcc{p} is concrete and
primitive, \vcc{\embedding(p)} is concrete.
\\\\
\vcc{size_t \sizeof_object(\object p)}\\
Equivalent to \vcc{\sizeof(T)}, where \vcc{T} is the type
of \vcc{*p}. This depends only on \vcc{\typeof(p)}. For any \vcc{p}, 
...
Every pointer of the program has a concrete footprint and a ghost
footprint (each a set of addresses), defined as follows.
If \vcc{p} is concrete/ghost primitive,
its concrete/ghost footprint is the set of addresses
\vcc{[\address(p),\address(p) + \sizeof_object(p))},
and its ghost/concrete footprint is empty. 
If \vcc{p} is an object, its concrete/ghost footprint is the union of
the footprints of all concrete/ghost primitive pointers 
\vcc{q} such that 
\vcc{\embedding(q) == p}. 
Intuitively, the concrete/ghost footprint of a pointer is that part of
the concrete/ghost heap used to store its representation.

Distinct primitive pointers with the same embedding do
not overlap.  Distinct concrete pointers have have disjoint ghost
footprints. If \vcc{p} is concrete and primitive, its embedding is
concrete. If \vcc{p} is a concrete object, its concrete footprint is a
subset of \vcc{[\address(p),\address(p) + \sizeof_object(p))}.
...
For object \vcc{o} with field \vcc{f}, \vcc{&o->f} is a pointer whose
embedding is \vcc{o}, whose type is the type of \vcc{f}, and which is
ghost iff \vcc{o} is ghost or \vcc{f} is ghost. If \vcc{f} is a
concrete field, \vcc{\address(&o->f) == \address(o) + offset},
where \vcc{offset} is the offset of field \vcc{f}. 

Every object has the following fields (all of which are ghost):
\\\\
\vcc{\bool o->\closed}\\
True if \vcc{o} is closed.
\\\\
\vcc{\object o->\owner}\\
The owner of \vcc{o}. 
\\\\
\vcc{\objset o->\owns}\\
The set of objects owned by \vcc{o}, when \vcc{o}
is \vcc{closed}. 
\\\\
\vcc{\bool o->\valid}\\
True off \vcc{o} is valid. 
...
The annotation of a function defines an implicit invariant for the
type of activations of the function; this invariant is a conjunction,
one conjunct for each value of its program counter, of the form ``if
control is here then this is true''. 
...
the header \vcc{<vcc.h>}.  If the C flag \vcc{VERIFY} is not set,
this header defines the macro \vcc{_()} to be the empty string,
causing all annotations to be removed in preprocessing.

Each VCC annotation is described here with an entry of the form
...
\begin{description}
\item[command] an annotation that acts syntactically as a statement;
...
\subsection{Pure Contexts}
A pure context is an annotation context within which the state is not
...
Note that fields starting with \vcc{\}
cannot have their addresses taken.
...
It is convenient to think of the state as storing the previous state of
an execution (at some well-known ghost address), and for every update
to the state to update this location appropriately. 
\\\\
\vcc{s->\last}\\
The previous state of \vcc{s}. (Not allowed in VCC annotations.)
\\\\
...
\legal(s) <==> \good(s->\last) ==> \forall \object o; \notchanged(o) || \invariant(o))

\admissible(o) <==>
        \forall(\state s; \legal(s) && \at(s,\unchanged(o)) ==> \at(s,\invariant(o)))
        \forall(\state s; \legal(s) && \at(s,!\unchanged(o)) ==> \at(\stutter(s),\invariant(o))
\end{VCC}
...
invariant is admissible (by reasoning about the type
definitions). (Execution is assumed to start in a state where all
objects are good.) 
...
can only be owned by a closed object. Threads own themselves.
...
Threads are considered objects in VCC, but the details of their type
are axiomatized. Part of the thread state is a local copy of 
the whole system state. As a thread executes, reads and writes of
...
when control reaches an \vcc{_(atomic)} or \vcc{_(atomic_read)}), the
...
Each function declaration effectively introduces a type

During execution, the thread state can be viewed as a stack of
activation records, one for each currently active function call made
by the thread. The state of each of these activation records includes
values for its program counter and each of its purely local
variables. Each record has an invariant that its state doesn't change
except when it's at the top of the stack, and that when its state
changes, the system state changes according to the operational
semantics of the step it is taking. 

Verification of a function implicitly constructs a global state
predicate for each control point of the function. The type of activation
...
\begin{VCC}
  typedef _(claimable) struct \claim {
    ...
    _(invariant \forall \object o; o \in s && \this->\closed ==>
...
\\\\
\vcc{\claim \make_claim(\objset s, \bool p)} (special)\\
\begin{enumerate}
\item Assert that \vcc{p} holds.
\item In \vcc{p}, replace each subterm of the
form \vcc{\when_claimed(e)} with the value of \vcc{e} in the
current state.
\item Let \vcc{c} be a claim with claimed property \vcc{p} and claimed
objects \vcc{o} for which \vcc{!c->\used && !c->\closed}. 
\item Assert the admissibility of \vcc{c}.
\item Atomically \vcc{_(wrap c)} and increment the \vcc{\claim_count}s of the objects of \vcc{c}. 
\item \vcc{_(assert p)}
\end{enumerate}

\noindent\vcc{\claim \upgrade_claim(\claim c1,..., bool p))} (command) \\
...
to \vcc{o->f}, but also assserts 
\vcc{\active_claim(c) && \claims(c,o->\closed)}.

Note: this should be generalized to volatile fields, where \vcc{c}
...
\\\\
\vcc{T \when_claimed(T e)} (special)\\
...
blocks. Formal parameters of a function are always purely local.

Note: The special treatment of purely local variables is likely to be
...
\subsection{Globals Variable Declarations}
...
could appear as a parameter of a ghost function). This has the effect
of declaring an out parameter of the function. The declared parameter
...
This has two effects. First, it implicitly \vcc{_(require)}s that the
listed objects are \vcc{\writable} (section ??)
and \vcc{\thread_local}. Second, at the beginning of the function
...
arbitrary value of the result type. (Note: such a value always exists,
...
\item its specification  must include a \vcc{_(decreases)} clause;
...
Note: VCC currently supports pure functions without function
bodies, but with postconditions of particular forms. However, the
restrictions on such functions are complex, and their use is deprecated.
\\\\
\vcc{_(def <decl>)}\\
...
involved are available at verification time. Therefore, soundness is
not guaranteed unless the bodies of all pure functions are available.
...
A call \vcc{v = f(args)}, where \vcc{v} and \vcc{args} are all pure
local variables, is translated as follows:

\begin{enumerate}
\item
assert the state is \vcc{\good}
\item
assert the preconditions  of \vcc{f} with actual parameters substituted for formal parameters
\item 
assert that all objects listed in the writes clauses are \vcc{\writable}
and \vcc{\mutable} or \vcc{\wrapped}
\item change the heap state arbitrarily
\item assume that every object that was \vcc{thread_local} prior to
the call and was not listed in the writes clauses of the function has
the same version as it did before state change 
\item change any \vcc{_(out)} parameters arbitrarily
\item assume the postconditions of the call
\item assume the state is good
\end{enumerate}	
...
function calls so that the behavior allowed by C is equivalent to the
behavior of a naive translation of the program, where actual parameters
are evaluated sequentially left-to-right. These conditions are roughly
the following: <TODO>
...
\vcc{_(unwrapping \object o1, \object o2, ...) writes block} (command)\\
where \vcc{writes} is a sequence of \vcc{_(writes)} annotations
and \vcc{block} is a program block without a
specification. Let \vcc{S} be the \vcc{\objset} be the set of objects
...
(\ref{??}), as a ghost declaration, or as a ghost
...
loop exit translates to a \vcc{goto} to an implicit label just after the
...
\vcc{_(assert \active_claim(o))} if \vcc{o} is a \vcc{\claim} and to
\vcc{_(assert o->\closed && \inv(o))} if it is not.
