...
the outermost level. We define three predicates:
\begin{itemize}
\item
\vcc{Oblig(B,P,L)}, where \vcc{B} is a restricted statement, \vcc{P}
is a predicate, and \vcc{L} is a function from labels to predicates.
...
\item
\vcc{Exit(B,P,l)}, giving what is guaranteed to hold
if \vcc{B} exits with a \vcc{goto} to label \vcc{l}.
\end{itemize}
...
is the precondition of the function.
At entry to the function, VCC knows the
precondition of the function, and that all object invariants hold. On
exit from the function, it is asserted that the postcondition of the
function holds.

We define these functions as follows. In each case, \vcc{Exit(B,P,l)}
is \vcc{\false} unless defined otherwise.
\begin{itemize} 
\vcc{Oblig(B;C, P, L) == 
  Oblig(B,P,L) && Oblig(C,Post(B,P,L), or(L,


\vcc{B} is \vcc{x = e}, where \vcc{e} is the application of a single
operator (other than \vcc{++} or \vcc{--}) to a
suitable number of purely local variables other than \vcc{x}, or 
application of \vcc{&} to a local variable:\\
\begin{VCC}
Post(B,P) == ((\exists x P) && x == e)}
...
\item
\vcc{B} is \vcc{x = f(<args>)} where \vcc{args} is  a list of purely local
...
\exists \state s; 
  \at(s,P)
  && <post with parameter substitution and \old(e) replaced by \at(s,e)>
...
\item
\vcc{B} is \vcc{l:} (declaring a label):  After \vcc{B} 
what is known is the disjunction of \vcc{P} and the disjunction, over
locations immediately preceding \vcc{goto l}, of what is known at each
such location.

\item 
\vcc{B} is \vcc{goto l} where \vcc{l} is a label that occurs syntactically
later in the function body: after \vcc{B}, what is known is \vcc{\false}.

\item
\vcc{_(atomic ...) <block>} where \vcc{<block>} is a simplified
block\\

\vcc@_(ghost B)@ where \vcc{B} is a simplified block\\
\vcc@_(atomic obs) B@ where \vcc{obs} is a sequence of purely local
variables and \vcc{B} is a simplified block\\
\vcc@_(ghost_atomic obs) B@ where \vcc{obs} is a sequence of purely
local variables and \vcc{B} is a simplified block
\vcc@if (v) B@
where \vcc{v} is a purely local variable and \vcc{B} is a simplified block;\\
...
Let \vcc{St} be a statement of one of the forms above, and let \vcc{P}
be what VCC knows just before \vcc{St}. Then 
\begin{itemize}

\item If \vcc{St} is \vcc{v = f(<args>)}, then after \vcc{St} VCC
knows 
\begin{VCC}
\exists \state S; \at(s,\me) == \me && differOnlyAt(s,\now(),w,\me) && post
\end{VCC}
where \vcc{w} is the writes set in the specification of \vcc{f}
and \vcc{post} is the postcondition in the specification of \vcc{f}
(both after substitution of formal parameters). The generated
obligation is


\item If \vcc{St} is \vcc{l:} then after \vcc{St}, VCC knows the
disjunction of \vcc{P} and the disjunction of what it knew before each
statement \vcc{goto l} preceding \vcc{St} in the function body.

\item If \vcc{St} is \vcc{goto l;} then after \vcc{St} VCC
knows \vcc{\false}.

\item
if \vcc{St} is @if (v) B@ then at entry to \vcc{B}, VCC knows 
\vcc{P && B}. If at the end of \vcc{B} VCC knows \vcc{P'}, then
after \vcc{St} VCC knows \vcc{(v ? P' : P)}
\item if \vcc{St} is \vcc@while (v) _(invariant p) _(writes s) {B'}@
then at entry to \vcc{B'} and after the block, VCC knows
\begin{VCC}
\exists \state S; \at(s,\me) == \me && \at(s, P) && p 
 && differOnlyAt(S,\now(),s,\me)
\end{VCC}
In addition, after \vcc{St} VCC knows \vcc{!v}.

\item If \vcc{St} is \vcc{_(assert p)} or \vcc{_(assume p)}, then
afte \vcc{St} VCC knows \vcc{P && p}.
...
obligation. Let \vcc{P} be what VCC knows before \vcc{St}; then
\begin{itemize}
\item if \vcc{St} is \vcc@while (v) _(invariant p) _(writes s) {B'}@
then there is a proof obligation \vcc{P || Q ==> p}, where \vcc{Q} is
what VCC knows at the end of \vcc{B'}.

\item If \vcc{St} is \vcc{v = f(<args>)}, then the generated proof
obligations are (1) \vcc{P ==> Q}, where \vcc{Q} is the precondition
of \vcc{f}, and (2) \vcc{P ==> w \subset ws}, where \vcc{w} is the set
of pointers in the writes set of \vcc{f}, and \vcc{ws} is the writes
set.
