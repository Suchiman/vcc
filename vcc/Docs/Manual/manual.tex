...
single abstract value.  Record types declared using the same syntax as
...
\\\\
\noindent\vcc{\pure_(by_claim :pure \claim c)} (cast)(thread)\\
...
For every global variable \vcc{v} of primitive type, there is a dummy object
...
typedef _(volatile_owns) struct Thread {
  ...
  _(invariant \forall \object o; \unchanged(\mine(o)) || \inv2(\currentActivation))
} Thread;
...
typedef struct f#Activation { // activation of function f
  ...
  _(invariant \on_unwrap(\this, \done && \inv2(\caller)))
