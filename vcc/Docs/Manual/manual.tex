...
VCC adds the following infix operators. Each has the given
associativity, and precedence that lies somewhere strictly between that
of the given C operators. The specified unicode characters can also be used.

\begin{tabbing}
operator \= unicode \= code \= associativity \= precedence\\
\\
\vcc{==>}\>right\>between \vcc{:?} and  \vcc{=})\\
\vcc{<==}\>left\>between \vcc{:?} and  \vcc{=})\\
\vcc{<==>}\>left\>between \vcc{:?} and  \vcc{=})\\
\vcc{\in}\>left\>\>\\
\vcc{\is}\>left\>\> \\
\vcc{\diff}\>\>\>\\
...
or \Def{object} types. Object types are compound (\vcc{struct}
...
is \vcc{==} to \vcc{0} and \vcc{\false} otherwise.
...
\vcc{T1[T2]}\\
The type of maps from type \vcc{T2} to type \vcc{T1}. 
vcc{T1} and \vcc{T2} must be primitive types, and \vcc{T2} must
...
. 
Note, however, that if \vcc{\approves(\this->\owner,f)} appears in 
